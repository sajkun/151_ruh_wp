...
class velesh_theme_posts {
  ...
  public static function add_leads_sub_menu_pages(){
    ...
    add_submenu_page( 'edit.php?post_type=lead_item', 'Email Configuration', 'Email Configuration', 'manage_options', 'email-configuration', array(__CLASS__, 'print_email_cb'));
    ...
    public static function print_email_cb(){
        if (isset($_POST['do_save']) && 'yes' == $_POST['do_save']) {
            update_option('email_data', $_POST['email_data']);
          }
       $clinics = get_option('clinics_list');
       $email_data = get_option('email_data');

       ?> <h3>Email configuration</h3> 
       <form action="<?php echo admin_url('edit.php?post_type=lead_item&page=email-configuration'); ?>" method="POST">
       <?php foreach ($clinics as $key => $clinic) {?>
        <h4 style="margin: 0">Email for <?php echo $clinic; ?></h4>
        <input type="email" required name='email_data[<?php echo  $clinic ?>][email]' autocomplete='no' value="<?php echo $email_data[$clinic]['email']?:''; ?>" /> <br><br>
        <h4 style="margin: 0">Password</h4>
        <input type="text" required autocomplete='no' value="<?php echo $email_data[$clinic]['password']?:''; ?>" name='email_data[<?php echo $clinic?>][password]'> <br><br>
        <?php } ?>
        <br/>
      ...
      ?> <h3>Twilio Configuration</h3>
        ...
        <table class="fullwidth">
            <tr>
                <th> Account SID </th>
                <td>
                    <input type="text" class="large-text" name="message_data[sid]"
                    value="<?php echo (isset($o['sid']))? $o['sid'] : ''; ?>">
                </td>
            </tr>
            <tr>
                <th> Auth token </th>
                <td>
                    <input type="text" class="large-text" name="message_data[token]"
                    value="<?php echo (isset($o['token']))? $o['token'] : ''; ?>">
                </td>
            </tr>
            <tr>
                <th> Phone Number </th>
                <td>
                    <input type="text" class="regular-text" name="message_data[phone]"
                    value="<?php echo (isset($o['phone']))? $o['phone'] : ''; ?>">
                </td>
            </tr>
        </table>
        <input type="hidden" name="do_save" value="yes">
...
<form action="<?php echo admin_url('users.php?page=role-configuration'); ?>" method="POST">
      ...
      <tr>
         ...
         <td>
            <select name="theme_roles[dentist]" class="roles-list" id="dentist">
               <option value="none">-- Select a role --</option> <?php foreach ($all_roles as $role => $data) {
   ...
   <p>Clinics were saved succesfully</p>
</div> <?php
        }else{
        ...
        if(isset($_POST['treatment'])){
         ...
         ?> <div class="notice notice-success is-dismissible">
   <p>Treatments were saved succesfully</p>
</div> <?php
        }else{
        ...
        if(isset($_POST['campaign'])){
         ...
         ?> <div class="notice notice-success is-dismissible">
   <p>Campaigns were saved succesfully</p>
</div> <?php
        }else{
        ...
        if(isset($_POST['sources'])){
         ...
         ?> <div class="notice notice-success is-dismissible">
   <p>Sources were saved succesfully</p>
</div> <?php
        }else{
        ...
        if(isset($_POST['payment_methods'])){
         ...
         ?> <div class="notice notice-success is-dismissible">
   <p>Payment Methods were saved succesfully</p>
</div> <?php
        }else{
        ...
        if(isset($_POST['tag_cloud'])){
         ...
         ?> <div class="notice notice-success is-dismissible">
   <p>Tag Cloud was saved succesfully</p>
</div> <?php
        }else{
      ...
      ?> <br><br>
<form action="<?php echo admin_url('edit.php?post_type=lead_item&page=clinics-treatment-configurations') ?>"
   method="POST">
   ...
   <div class="page-container clinics">
      <div class="clinics-list"> <?php if ($clinics): ?> <?php foreach ($clinics as $key => $cl):
                if(!$cl) continue; ?> <div class="input-control">
            <p class="input-title">Clinic #<?php echo $key ?> <a href="javascript:void(0)"
                  <?php echo 'style="float:right"' ?>onclick="delete_input(this)">Delete Clinic</a></p>
   ...
   <div class="page-container treatment">
      <div class="treatments-list" <?php echo 'style="column-count:3; column-gap:20px"' ?>> <?php if ($treatments):
                $counter = 0;
                ?> <?php foreach ($treatments as $key => $cl):
                if(!$cl) continue?> <div class="input-control">
            <p class="input-title">Treatment #<?php echo $counter;  ?> <a href="javascript:void(0)"
                  <?php echo 'style="float:right"' ?> onclick="delete_input(this)">Delete treatment</a></p>
   ...
   <div class="page-container campaign">
      <div class="campaigns-list"> <?php if ($campaigns):
               $counter = 0;
              ?> <?php foreach ($campaigns as $key => $cl):
            if(!$cl) continue ?> <div class="input-control">
            <p class="input-title">Campaign #<?php echo $counter ?> <a href="javascript:void(0)"
                  <?php echo 'style="float:right"' ?> onclick="delete_input(this)">Delete Campaign</a></p>
   ...
   <div class="page-container sources">
      <div>
         <label>
            ...
            echo 'yes' ==  $add_dentists_to_sources?'checked="checked"' : ''; ?> name="add_dentists_to_sources"
               value="yes">Include dentists to sources from user roles</label>
      </div>
      <br>
      <div class="sources-list" <?php echo 'style="column-count:2; column-gap:20px"' ?>> <?php if ($sources):
                $counter = 0;
                ?> <?php foreach ($sources as $key => $cl): ?> <div class="input-control">
            <p class="input-title">Source #<?php echo $counter ?> <a href="javascript:void(0)"
                  <?php echo 'style="float:right"' ?> onclick="delete_input(this)">Delete Source</a></p>
   ...
   <div class="page-container payment">
      <div class="payment_methods-list"> <?php if ($payment_methods):
               $counter = 0;
              ?> <?php foreach ($payment_methods as $key => $cl):
              if(!$cl) continue;
              ?> <div class="input-control">
            <p class="input-title">Payment Method #<?php echo $counter ?> <a href="javascript:void(0)"
                  <?php echo 'style="float:right"' ?> onclick="delete_input(this)">Delete method</a></p>
      ...
      <input type="hidden" id="count_payment_methods"
         value="<?php echo  $payment_methods? count( $payment_methods): 0 ?>">
   </div>
   <div class="page-container tag">
      <div class="tag-list" <?php echo 'style="column-count:3; column-gap:20px"' ?>> <?php if ($tag_cloud):
                $counter = 0;
                ?> <?php foreach ($tag_cloud as $key => $cl):
                if(!$cl) continue?> <div class="input-control">
            <p class="input-title">Tag #<?php echo $counter;  ?> <a href="javascript:void(0)"
                  <?php echo 'style="float:right"' ?> onclick="delete_input(this)">Delete tag</a></p>
   ...
   <input type="hidden" name="do_save" value="yes">
    ...
    public static function print_leads_stages_cb(){
      ...
      if(isset($_POST['save_leads_stages']) && 'yes' === $_POST['save_leads_stages']){
        ...
        if($updated){
          ?> <div class="notice notice-success is-dismissible">
   <p>Changes were saved succesfully</p>
</div> <?php
        }
   ...
   <p>No stage is selected as stage for converted leads</p>
</div> <?php
        }
        ...
        if(isset($_POST['stage_for_failed'])){
          ...
          ?> <div class="notice notice-error is-dismissible">
   <p>No stage is selected as stage for failed leads</p>
</div> <?php
        }
      }
...
<form action="<?php echo admin_url('edit.php?post_type=lead_item&page=leads-stages') ?>" method="POST">
   ...
   <div class="stages-content">
      <ul class="stages-content__list"> <?php
              $number = 0;
              if($stages):
               foreach ($stages as $key => $st): ?> <li>
            <div class="stages-content__item">
               <table>
                  <tr>
                     <th colspan="4">
                        <h3>Stage #<span class="number"><?php echo ($number + 1) ?></span></h3>
                        <input type="hidden" name="leads_stages[<?php echo ($number) ?>][number]"
                           value="<?php echo ($number) ?>" class="stage_order">
                        <a href="javasctipt:void(0)" onclick="delete_leads_stage(this);">Delete Stage</a>
                     </th>
                  </tr>
                  <tr>
                     <th>Name</th>
                     <td colspan="3"><input type="text" class="regular-text"
                           name="leads_stages[<?php echo ($number) ?>][name]" value="<?php echo $st['name'] ?>"></td>
                  </tr>
                  <tr>
                     <th>Background Color</th>
                     <td><input type="text" class="short-text colorpicker"
                           name="leads_stages[<?php echo ($number) ?>][bg_color]" value="<?php echo $st['bg_color'] ?>">
                     </td>
                     <th>Text Color</th>
                     <td><input type="text" class="short-text colorpicker"
                           name="leads_stages[<?php echo ($number) ?>][text_color]"
                           value="<?php echo $st['text_color'] ?>"></td>
                  </tr>
                  <tr>
                     <td colspan="4">
                        <label class="optional_label">
                           <input type="radio" name="stage_for_converted" class="stage_for_converted"
                              value="<?php echo ($number) ?>"
                              <?php echo  $stage_for_converted && (int)$number === (int)$stage_for_converted? 'checked="checked"': ''  ?>>
                           Is converted <br>
                           <i>Leads on this stage will be counted as converted</i>
                        </label>
                        <label class="optional_label">
                           <input type="checkbox" name="stage_for_failed[]" class="stage_for_failed"
                              value="<?php echo ($number) ?>"
                              <?php echo is_array($stage_for_failed) && in_array($number, $stage_for_failed)? 'checked="checked"': ''  ?>>
                           Is failed <br>
                  ...
                  <tr>
                     ...
                     <td colspan="3" class="th">
                        <label>
                           ...
                           <input type="checkbox" value="1" name="leads_stages[<?php echo ($number) ?>][reception]"
                              <?php echo  isset($st['reception']) &&  1 == $st['reception'] ? 'checked="checked"' : ''; ?>>
                           Reception </label>
                        <label>
                           <input type="hidden" value="0" name="leads_stages[<?php echo ($number) ?>][tco]">
                           <input type="checkbox" value="1" name="leads_stages[<?php echo ($number) ?>][tco]"
                              <?php echo  isset($st['tco']) &&  1 == $st['tco'] ? 'checked="checked"' : ''; ?>> TCO
                        </label>
                     </td>
                  </tr>
               </table>
  ...
  public static function lead_stage_cb($post){
    ...
    ?> <select name="lead_stage" id="lead_stage" class="fullwidth"> <?php foreach ($stages as $key => $st): ?> <option
      value="<?php echo $st['name']?>" <?php echo $meta === $st['name'] ||$select_first? 'selected="selected"' : '';?>>
  ...
  public static function lead_reminder_cb($post){
    ...
    ?> <input type="text" autocomplete="off" name="reminder" placeholder="Add"
   class="leads-block__input fullwidth datetimepicker" value="<?php echo isset($meta)? $meta : ''; ?>">
<br> <br>
<a href="javascript:void(0)" onclick="clear_input(this)">Clear Reminder</a> <?php
  }
  ...
  public static function lead_patient_data_cb($post){
   ...
   <div class="leads-block__name">
      <h4>Name</h4>
      <input type="text" name="patient_data[name]" value="<?php echo isset($meta['name']) ? $meta['name'] : ''; ?>"
         placeholder="Enter Name" class="leads-block__input lg">
   </div>
   <br>
   <table class="leads-block__data fullwidth" <?php echo 'style="width: 100%"'; ?>>
      <tbody>
         <tr>
            ...
            <td width="200" <?php echo 'style="width:200px"'; ?>>
               <p class="leads-block__label">Phone</p>
            </td>
            <td>
               <input type="text" name="patient_data[phone]"
                  value="<?php echo isset($meta['phone'])? $meta['phone'] : ''; ?>" placeholder="Add"
                  class="leads-block__input fullwidth">
            </td>
         </tr>
         <tr>
            <td>
            ...
            <td>
               <p class="leads-block__label">E-mail</p>
            </td>
            <td>
               <input type="text" name="patient_data[email]" placeholder="Add" class="leads-block__input fullwidth"
         ...
         <tr>
            ...
            ?> <select name="patient_data[source]" id="patient_data[source]" class="fullwidth">
                  <option value="-1">Select</option> <?php foreach ($sources as $key => $s): ?> <option
                     value="<?php echo $s ?>"
                     <?php echo isset($meta['source']) && $s == $meta['source']?'selected="selected"' : ''; ?>>
                     <?php echo $s ?></option> <?php endforeach ?>
               </select>
            </td>
         </tr>
         <tr>
            ...
            <td>
               <p class="leads-block__label">Treatment</p>
            </td>
            <td>
               <select name="patient_data[treatment]" id="patient_data[treatment]" class="fullwidth">
                  <option value="">Select</option> <?php foreach ($treatments as $s): ?> <option
                     value="<?php echo $s ?>"
                     <?php echo isset($meta['treatment']) && $s == $meta['treatment']?'selected="selected"' : ''; ?>>
                     <?php echo $s ?></option> <?php endforeach ?>
               </select>
            </td>
         </tr>
         <tr>
            ...
            <td>
               <select name="patient_data[clinic]" id="patient_data[clinic]" class="fullwidth">
                  <option value="">Select</option> <?php foreach ($clinics as $s): ?> <option value="<?php echo $s ?>"
                     <?php echo isset($meta['clinic']) && $s == $meta['clinic']?'selected="selected"' : ''; ?>>
                     <?php echo $s ?></option> <?php endforeach ?>
               </select>
            </td>
         </tr>
         <tr>
            <td>
               <svg class="icon svg-icon-date">
                  <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-icon-date"></use>
               </svg>
            </td>
            <td>
               <p class="leads-block__label">Date / Time</p>
            </td>
            <td> <?php
               $date = new DateTime($post->post_modified);
             ?> <input type="text" readonly name="patient_data[date_time]" placeholder="Add"
                  class="leads-block__input fullwidth" value="<?php echo $date->format('M d Y H:i') ?>">
            </td>
         </tr>
         <tr>
            <td>
            ...
            <td> <?php
             ?> <input type="text" name="patient_data[campaign]" placeholder="Add" class="leads-block__input fullwidth"
...
<input type="hidden" name="save_theme_meta" value="yes"> <?php
  }
  ...
  public static function lead_proposed_treatment_value_cb($post){
   ...
   <div class="leads-block__price">
      <input type="text" name="treatment_value[value]" placeholder="£00.00" class="leads-block__input xxl price_input"
         value="<?php echo isset($meta['value']) ? $meta['value'] : ''; ?>">
   </div>
...
<div class="leads-block__row">
   <table class="leads-block__data fullwidth">
      <tbody>
         <tr>
            <td>
               <svg class="icon svg-icon-clock green">
                  <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-icon-clock"></use>
               </svg>
            </td>
            <td>
               <p class="leads-block__label">Payment Terms</p>
            </td>
            <td>
               <select name="treatment_value[terms]" id="treatment_value[terms]" class="terms_select fullwidth">
                  <?php foreach ($terms as $key => $s): ?> <option value="<?php echo $s ?>"
                     <?php echo isset($meta['terms']) && $s == $meta['terms']?'selected="selected"' : ''; ?>>
                     <?php echo $s ?></option> <?php endforeach ?> </select>
            </td>
         </tr>
         <tr>
            <td>
               <svg class="icon svg-icon-monthly green">
                  <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-icon-monthly"></use>
               </svg>
            </td>
            <td>
               <p class="leads-block__label">Monthly</p>
            </td>
            <td>
               <input type="text" readonly name="treatment_value[mounthly]" placeholder="Add"
                  class="leads-block__input fullwidth treatment_mounthly"
                  value="<?php echo isset($meta['mounthly']) ? $meta['mounthly'] : ''; ?>">
            </td>
         </tr>
         <tr>
            <td>
               <svg class="icon svg-icon-card green">
                  <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-icon-card"></use>
               </svg>
            </td>
            <td>
               <p class="leads-block__label">Payment Method</p>
            </td>
            <td>
               <input type="text" name="treatment_value[payment_method]" placeholder="Add"
                  class="leads-block__input fullwidth"
                  value="<?php echo isset($meta['payment_method']) ? $meta['payment_method'] : ''; ?>">
            </td>
         </tr>
         <tr>
            <td>
               <svg class="icon svg-icon-tooth green">
                  <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-icon-tooth"></use>
               </svg>
            </td>
            <td>
   ...
   <table class="leads-block__data fullwidth">
      <tbody>
         <tr>
            ...
            <td>
               <p class="leads-block__label">Consultation Date</p>
            </td>
            <td>
               <input type="text" name="treatment_coordinator[consultation_date]" placeholder="Add"
                  class="leads-block__input fullwidth datetimepicker"
                  value="<?php echo isset($meta['consultation_date']) ? $meta['consultation_date'] : ''; ?>">
            </td>
         </tr>
         <tr>
            <td>
               <svg class="icon svg-icon-lamp">
                  <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-icon-lamp"></use>
               </svg>
            </td>
            <td>
               <p class="leads-block__label">Reason for Appt.</p>
            </td>
            <td>
               <input type="text" name="treatment_coordinator[reason]" placeholder="Add"
                  class="leads-block__input fullwidth"
                  value="<?php echo isset($meta['reason']) ? $meta['reason'] : ''; ?>">
            </td>
         </tr>
         <tr>
            <td>
               <svg class="icon svg-icon-leaps">
                  <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-icon-leaps"></use>
               </svg>
            </td>
            <td>
               <p class="leads-block__label">Dentist Seen</p>
            </td>
            <td>
               <input type="text" name="treatment_coordinator[specialist]" placeholder="Add"
                  class="leads-block__input fullwidth"
                  value="<?php echo isset($meta['specialist']) ? $meta['specialist'] : ''; ?>">
            </td>
         </tr>
         <tr>
            <td>
               <svg class="icon svg-icon-chat">
                  <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-icon-chat"></use>
               </svg>
            </td>
            <td>
               <p class="leads-block__label">Follow Up</p>
            </td>
            <td>
               <input type="text" name="treatment_coordinator[follow]" placeholder="Add"
                  class="leads-block__input fullwidth"
                  value="<?php echo isset($meta['follow']) ? $meta['follow'] : ''; ?>">
            </td>
         </tr>
      </tbody>
   </table>
...
<input type="hidden" name="save_theme_meta" value="yes"> <?php
  ...
  public static function lead_documents_cb($post){
    ...
    $count = 0;
    ?> <div class="uploaded-documents"> <?php
        if($meta):
      ...
      <p class="document-block__actions">
         <a href="<?php echo $m['url'] ?>" download <?php echo 'style="text-decoration: none;"'; ?>><svg
               class="icon svg-icon-download">
               <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-icon-download"></use>
            </svg> </a>
         <svg class="icon svg-icon-trash" onclick="delete_file(this)">
      ...
      <input type="hidden" name="lead_files[<?php echo $count;?>][url]" value="<?php echo $m['url'] ?>">
         ...
         ?> </div>
<?php /*<a href="javascript:void(0)" class="button" onclick="upload_document(this)">Upload Document</a> */ ?> <input
   type="hidden" id="files_count" value="<?php echo $count; ?>">
<input type="hidden" name="save_theme_meta" value="yes"> <?php
  }
  ...
  public static function lead_notes_cb($post){
    ...
    ?> <div class="notes-block"> <?php
      ...
      <div class="note-block__body"> <?php echo $m['text'] ?> </div>
...
<input type="hidden" name="save_theme_meta" value="yes">
<input type="hidden" id="user_nice_name" value="<?php echo $user_name; ?>"> <?php
  }
  ...
  public static function lead_specialists_cb($post){
   ...
   <tbody> <?php foreach ($users as $key => $user):
          ...
          ?> <tr>
         <td>
            <div class="team-leads__photo"><img src="<?php echo $image ?>" alt=""></div>
         </td>
         <td colspan="3">
            <div class="clearfix">
         ...
         <td>
            ...
            <input type="checkbox" name="lead_specialists[<?php echo $user->ID ?>]" value="yes"
               <?php echo $meta[$user->ID] === 'yes'? 'checked="checked"' : '' ?>>
         </td>
...
<input type="hidden" name="save_theme_meta" value="yes"> <?php
  ...
  public static function lead_logs_cb($post){
   ...
   <li>
      <i class="state-none icon-activity"></i> <span class="leads-block__activity-text"><span class="action">Lead
            Created</span> <span class="date"><?php echo $date->format('d M Y') ?> at <?php echo $date->format('H:I') ?>
         </span></span> <span class="length">0d 0h 0m</span>
   </li> <?php if ($meta): ?> <?php foreach ($meta as $key => $d): ?> <li>
      <i class="state-none icon-activity"></i> <span class="leads-block__activity-text"><span
            class="action"><?php echo $d['text'] ?></span> <span class="date"><?php echo $d['date_formatted'] ?>
         </span></span> <span class="length"><?php echo $d['time_passed'] ?></span>
   </li> <?php endforeach ?> <?php endif ?>
</ul>
<input type="hidden" name="lead_logs" value="yes"> <?php
  }
