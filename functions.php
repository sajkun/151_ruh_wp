...
class velesh_init_theme
{
  /* main style location  */
    ...
    public $main_script = '/assets/script/main.min40.js';

    public $main_script_spa = '/assets/script/main.spa.min40.js';
    ...
    public function __construct()
    {
        ...
        if ($this->is_request('frontend')) {
            ...
            add_action('wp_head', ['theme_construct_page', 'init']);
        }
    ...
    public function define_theme_globals()
    {
        ...
        define('RELOAD_LEAD', false); // @deprecated don't change
    ...
    public function enqueue_scripts_styles_front()
    {
        ...
        wp_enqueue_script('theme-chart-js', 'https://cdn.jsdelivr.net/npm/chart.js@2.8.0', ['jquery'], THEME_VERSION, true);
    ...
    public function enqueue_scripts_styles_admin()
    {
        ...
        wp_enqueue_script('theme-date-time-picker', THEME_URL . '/assets/libs/datetimepicker/build/jquery.datetimepicker.full.min.js', ['jquery', 'wp-color-picker'], THEME_VERSION, true);

        add_svg_sprite($this->svg_sprite_slug, THEME_URL . '/assets/svg_sprite/symbol_sprite.html');

        wp_enqueue_script('theme-script', THEME_URL . '/assets/script/admin2.js', ['jquery'], THEME_VERSION, true);
    ...
    {
        add_action('wp_enqueue_scripts', [$this, 'enqueue_scripts_styles_front'], 9991);

        add_action('wp_enqueue_scripts', [$this, 'prepare_template_data'], 9992);

        add_action('wp_enqueue_scripts', [$this, 'inline_custom_data'], 9990);

        add_action('do_theme_after_head', [$this, 'print_theme_inline_styles'], 9999);

        add_filter('script_loader_tag', [$this, 'add_async_attribute'], 10, 2);

        /* js and css hooks for the admin dashboard*/

        add_action('admin_enqueue_scripts', [$this, 'enqueue_scripts_styles_admin'], 5);

        add_action('admin_enqueue_scripts', [$this, 'inline_custom_data'], 13);

        /* theme setup actions */

        add_action('after_setup_theme', [$this, 'setup_theme']);

        add_filter('upload_mimes', [$this, 'cc_mime_types'], 10);

        add_action('theme_start_page_header', [$this, 'print_inline_data_body']);

        add_action('init', [$this, 'add_cors_http_header']);

        add_action('wp_footer', [$this, 'print_styles_in_footer']);

        add_action('admin_init', [$this, 'add_reading_settings']);

        add_action('admin_menu', [$this, 'add_option_pages']);

        add_action('init', [$this, 'manager_cant_see_dashboard']);
    ...
    */
    ...
    {
        add_settings_section('theme-pages-section', __('Custom page settings', 'theme-translations '), [$this, 'add_additional_page_settings'], 'reading');
    }
