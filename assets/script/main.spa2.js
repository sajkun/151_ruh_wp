...
Vue.component('comp-single-lead', {
    ...
    watch: {
        ...
        text_messages_to_show: function (val) {
            if (val > 2) {
                Vue.nextTick(function () {
                    jQuery('._messages')[0].scrollTop =
                        jQuery('._messages')[0].scrollHeight
                })
            }
        },
        ...
        note_text: function () {
            this.$refs.note_textarea.style.height = ''
            this.$refs.note_textarea.style.height =
                this.$refs.note_textarea.scrollHeight + 'px'
        },
    ...
    computed: {
        ...
        deactivate_text: function () {
            return this.lead_data.meta.deactivated_lead === 'yes'
                ? 'Activate'
                : 'Deactivate'
        },
        ...
        email_log: function () {
            ...
            var logs = this.lead_data.meta.email_log.map((e) => {
                ...
                var fmt = new DateFormatter()
                e.date_formatted =
                    fmt.formatDate(date, 'F d Y') +
                    ' at ' +
                    fmt.formatDate(date, 'H:ia')
        ...
        text_messages_shown: function () {
            if (
                this.text_messages_to_show == 2 &&
                this.text_messages.length > 2
            ) {
                var messages = []
                for (var i = 2; i > 0; i--) {
                    messages.push(
                        this.text_messages[this.text_messages.length - i]
                    )
                }
                return messages
            } else {
        ...
        treatment_data: function () {
            return this.lead_data.meta.treatment_data
                ? this.lead_data.meta.treatment_data
                : []
        },
        ...
        visible_specialists: function () {
            ...
            if ('object' == typeof this.lead_data.meta.specialists_assigned) {
                for (var _id in this.lead_data.meta.specialists_assigned) {
                    if (
                        this.lead_data.meta.specialists_assigned[_id] == 'yes'
                    ) {
        ...
        visible_specialists_tco: function () {
            ...
            if (
                'object' == typeof this.lead_data.meta.specialists_assigned_tco
            ) {
                for (var _id in this.lead_data.meta.specialists_assigned_tco) {
                    if (
                        this.lead_data.meta.specialists_assigned_tco[_id] ==
                        'yes'
                    ) {
                        ids.push(parseInt(_id))
                    }
    ...
    methods: {
        add_note: function (type) {
            ...
            if (type == 'enquery') {
                if (
                    !this.lead_data.meta.lead_notes ||
                    this.lead_data.meta.lead_notes == 'false'
                ) {
        ...
        add_treatment_dentist: function () {
            ...
            Vue.nextTick(function () {
                ...
                if (is_dentist === 'yes') {
                    vm.$refs['select_dentist'][select_id].set_value(
                        'selected',
                        dentist_name
                    )
                }
        ...
        change_image_uploaded_cb: function (event, name) {
            var path =
                this.lead_data.meta.online_journey.dropbox_path +
                event.file.name
        ...
        deactivate_lead: function () {
            ...
            var data = {
                ...
                value:
                    this.lead_data.meta.deactivated_lead == 'no' ? 'yes' : 'no',
        ...
        remove_specialist: function (user_id, type) {
            if (
                window.confirm(
                    'Confirm unassigning ' + name + ' from this lead'
                )
            ) {
                switch (type) {
                    case 'tco':
                        this.lead_data.meta.specialists_assigned_tco[user_id] =
                            'no'
                        break
                    default:
        ...
        save_specialists_meta: function () {
            ...
            for (id in specialists_data) {
                meta[specialists_data[id].user_id] = this.lead_data.meta
                    .specialists_assigned[specialists_data[id].user_id]
                    ? this.lead_data.meta.specialists_assigned[
                          specialists_data[id].user_id
                      ]
                    : 'no'
                meta_tco[specialists_data[id].user_id] = this.lead_data.meta
                    .specialists_assigned_tco[specialists_data[id].user_id]
        ...
        save_lead_meta: function (key_meta, key_this) {
            ...
            if (typeof key_meta !== 'string') {
                var meta = {
                    ...
                    treatment_coordinator:
                        this.lead_data.meta.treatment_coordinator,
            ...
            for (var _filter_data in filter_data) {
                ...
                vm.$parent.$set(
                    vm.$parent.leads[index].filter_data,
                    _filter_data,
                    value
                )
                vm.$parent.$set(
                    vm.$parent.leads[index].filter_data2,
                    _filter_data,
                    value
                )
            }
        ...
        treatment_data_selects: function () {
            ...
            for (var id in vm.lead_data.meta.treatment_data) {
                ...
                vue_select_components.push(
                    vm.$refs['select_dentist'][select_id]
                )
                ...
                for (var i in props) {
                    vm.$refs['select_treatment'][select_id].set_value(
                        i,
                        props[i]
                    )
                }
                ...
                vue_select_components.push(
                    vm.$refs['select_treatment'][select_id]
                )
                ...
                for (var i in props) {
                    vm.$refs['select_payment_method'][select_id].set_value(
                        i,
                        props[i]
                    )
                }
                ...
                vue_select_components.push(
                    vm.$refs['select_payment_method'][select_id]
                )
                vm.$refs.select_billed[select_id].set_value(data['billed'])
        ...
        update_treatment_data: function (e, key) {
            if (typeof e.val !== 'undefined') {
                ...
                for (var id in this.lead_data.meta.treatment_data) {
                    total += get_sum_from_price(
                        this.lead_data.meta.treatment_data[id].billed
                    )
                }
        ...
        update_lead: function (data, key, id) {
            if ('object' === typeof data) {
                if (
                    ...
                    if (
                        'undefined' ===
                        typeof this.lead_data.meta[key][data.name]
                    ) {
                        this.lead_data.meta[key][data.name] = []
                    }

                    if (
                        this.lead_data.meta[key][data.name].indexOf(data.val) <
                        0
                    ) {
                        this.lead_data.meta[key][data.name].push(data.val)
                    } else {
                        var ind = this.lead_data.meta[key][data.name].indexOf(
                            data.val
                        )
        ...
        update_specialists: function (event, type) {
            if ('undefined' !== typeof event.val) {
                if (this.lead_data.ID < 0) {
                    alert(
                        'Create lead before assigning it to a specialist, please'
                    )
                    return false
                }
                ...
                if (do_save) {
                    ...
                    var team_filter_data = Object.values(
                        this.$parent.leads[lead_index].filter_data.team
                    )
        ...
        upload_image_to_dropbox: function (file, path, name) {
            ...
            xhr.setRequestHeader(
                'authorization',
                'Bearer ' + online_journey_settings.token
            )
        ...
        remove_file: function (file_id) {
            ...
            if (
                window.confirm(
                    'Confirm deleting file ' + this.files[file_id].name
                )
            ) {
                var file_data = vm.files[file_id]
        ...
        change_phone_tco: function (action) {
            ...
            this.save_parent_meta(
                this.lead_data.ID,
                this.lead_data.phone_count_tco,
                'phone_count_tco'
            )
        ...
        date_difference: function (key) {
            ...
            if (key + 1 < this.lead_data.meta.lead_stage_log2.length) {
                $start_date =
                    key == -1
                        ? this.lead_data.post_date
                        : this.lead_data.meta.lead_stage_log2[key].date

                $start_date = new Date($start_date)
                $end_date = new Date(
                    this.lead_data.meta.lead_stage_log2[key + 1].date
                )
