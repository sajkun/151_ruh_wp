	...
	computed: {
		...
		select_data: function () {
			return {
				...
				clinics: ['London Fleet Street', 'London Notting Hill', 'Manchester'],
			}
		},
		...
		validate_email: function () {
			...
			switch (vm.template) {
				case 'Booked In':
					...
					var valid_price = vm.template_data['Booked In'].price != '' ? true : false
...
Vue.component("comp-single-lead", {
  ...
  computed: {
    ...
    email_log: function () {
      ...
      var logs = this.lead_data.meta.email_log.map((e) => {
        ...
        var fmt = new DateFormatter();
        e.date_formatted =
          fmt.formatDate(date, "F d Y") + " at " + fmt.formatDate(date, "H:ia");
  ...
  methods: {
    add_note: function (type) {
      ...
      var fmt = new DateFormatter();
      var date_formatted =
        fmt.formatDate(date, "F d Y") + " at " + fmt.formatDate(date, "H:i");
    ...
    change_image_uploaded_cb: function (event, name) {
      var path =
        this.lead_data.meta.online_journey.dropbox_path + event.file.name;
      ...
      if ("undefined" !== typeof event.val) {
        ...
        switch (type) {
          case "tco":
            ...
            if (
              this.lead_data.meta.specialists_assigned_tco[data.user_id] !==
              "yes"
            ) {
              this.$set(
                this.lead_data.meta.specialists_assigned_tco,
    ...
    value_to_price: function (ref) {
      switch (ref) {
        case "price_input_field":
          var summ =
            "£" +
            formatMoney(this.lead_data.meta.treatment_value.value, 2, ".", ",");
          break;
        case "input_billed":
          var summ =
            "£" +
