...
function do_sort(){
    ...
    jQuery("#leads-list").find( ".leads-list" ).on( "sortreceive", function( event, ui ) {
      ...
      jQuery("#leads-list").trigger('save_dragged_item', {post_id: post_id, list_id: list_id})
...
jQuery("#leads-list").on('update_items_order', function(e, data){
...
jQuery("#leads-list").on('save_dragged_item', function(e, data){
...
// }
console.log(typeof(is_lead_list_2));
  ...
  list_app = new Vue({
    ...
    computed: {
      ...
      get_convertion: function(){
        ...
        return function (col_id) {
          ...
          var column_number = this.stages_reception.findIndex(e=> {return e.name == col_id});
          ...
          if(col_id == failed_lead_name ){
            ...
            for(var i = 0 ; i <= column_number; i++){
              var _col_id = this.stages_reception[i].name;
      ...
      leads_by_column: function(){
        ...
        for(var lead_stage in leads){
          ...
          var stage_data = this.stages.filter(el=>{
            return el.name == lead_stage;
          });

          leads[lead_stage] = this.leads_filtered.filter(function(el){
            return el.lead_stage == lead_stage;
          });

          switch(this.sortby){
            case 'Recent Messages':
               leads[lead_stage].sort(this.sort_by_sms);
              break;
            case 'Date Added':
                leads[lead_stage].sort(this.sort_by_date_added);
              break;
            case 'Recently Updated':
              leads[lead_stage].sort(this.sort_by_date);
              break;
          }
        }
      ...
      */
      ...
      overdue_data: function(){
        var leads_reminder = this.leads.filter(el=>{return !!el.meta.reminder})
    ...
    methods: {
      ...
      adjust_sizes: function(){
        ...
        var width = vm.stages_reception.length * 300;
...
Vue.component('comp-single-lead', {
  ...
  computed: {
      ...
      enquery_notes_count : function(){
        return 0;
      },

      tco_notes_count  : function(){
        return 0;
      },
  ...
  methods: {
    ...
    update_treatment_data: function(e, key){
      if(typeof(e.val)  !== 'undefined'){
        ...
        this.lead_data.meta.treatment_data.value =  '£' + formatMoney(total,2, '.',',');
      }
    },
