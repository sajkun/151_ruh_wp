      ...
      */
      leads_filtered: function(){
        ...
        leads = leads.filter(function(el){
          ...
          if(vm.not_overdue_checked && (!el.meta.reminder || el.meta.reminder =='false' || overdue_date < now)){
            validated = false;
          }

          if(vm.overdue_only_checked && (!el.meta.reminder  || el.meta.reminder =='false' || overdue_date > now)){
            validated = false;
          }

          // show only with latest sms from clients
          if(vm.show_not_read_only && (!el.show_message_alert_him || parseInt(el.meta.disable_sms) == 1) ){
            validated = false;
          }
      ...
      */
      ...
      update_order_status_on_drag_cb: function(data){
        ...
        if(converted_stages.indexOf(this.leads[index].lead_stage) >=0){
         ...
         var lead_stage_log = strip(this.leads[index].meta.lead_stage_log);

         lead_stage_log.push({'stage' : this.leads[index].lead_stage, 'date' : fmt.formatDate(today, 'Y-m-d H:i:s'), by: theme_user_name})

         this.$set(this.leads[index]['meta'], 'lead_stage_log', lead_stage_log);
...
Vue.component('comp-single-lead', {
  ...
  methods: {
    ...
    change_stage_popup_cb:function(data){
      ...
      var lead_stage_log = strip(this.lead_data.meta.lead_stage_log);
      lead_stage_log.push({'stage' : data.stage, 'date' : fmt.formatDate(today, 'Y-m-d H:i:s'), by: theme_user_name})
      this.$set(this.lead_data.meta, 'lead_stage_log', lead_stage_log);

      this.save_lead_meta('lead_stage_log', 'lead_stage_log');
    },
    ...
    update_lead_stage: function(data, key){
      ...
      this.lead_data.lead_stage = data.val;
       ...
       var lead_stage_log = strip(this.lead_data.meta.lead_stage_log);
       lead_stage_log.push({'stage' : data.val, 'date' : fmt.formatDate(today, 'Y-m-d H:i:s'), by: theme_user_name})
       this.$set(this.lead_data.meta, 'lead_stage_log', lead_stage_log);
    },
    ...
    */
    ...
    date_difference: function(key){
      ...
      if('undefined' == typeof(this.lead_data.meta.lead_stage_log)){
        return false;
      }

      if((key + 1) < this.lead_data.meta.lead_stage_log.length){

       $start_date = ( key == -1)? this.lead_data.post_date : this.lead_data.meta.lead_stage_log[key].date;

       $start_date = new Date($start_date);
       $end_date = new Date(this.lead_data.meta.lead_stage_log[key+1].date);
       ...
       return  diff_days > 0? 'Passed '+ diff_days +'d' : diff_hours > 0?  'Passed '+ diff_hours +'h' :  diff_minutes > 0 ? 'Passed '+ diff_minutes +'h' : 'Less than a minute';
